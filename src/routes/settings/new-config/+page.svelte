  ...
  <div class="container-row center">
    <div class="container">
      <div class="row g-1" style="justify-content: center;">
        ...
        <InfoTip>Use any format you like, it must be unique. This will be how you define what model a phone is when you add them later.</InfoTip>
      </div>
    </div>
  </div>
  <div class="grid-wrapper" style="margin-right: -20rem;">
    {#each files as file, i}
      <div class="container-row center" style="grid-column: 1;">
        <div class="container fit">
          {#if i >= 1}
            <button class="btn-close" onclick={() => removeFile(i)}>✕</button>
          {/if}
          <div class="row g-1" style="justify-content: center">
            <Input id={`input-${i}`} label="Filename:" placeholder={"file_name.cfg"} bind:value={file.filename} error={errorMap[`input-${i}`]} required />
            <InfoTip
